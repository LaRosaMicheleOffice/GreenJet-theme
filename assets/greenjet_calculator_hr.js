...
function checkShower(value, local_view_id){
    ...
    if(value <= 0 || Number.isInteger(value) === false){
        alert("Broj mora biti najmanje 1");
        value = 1;
    } 
...
function readCheckBox(local_view_id){
    ...
    if (user_input[`connection${local_view_id}`] > 0){
        ...
        alert("Odaberite barem jednu vezu!");
    }
}
...
function setExplanation(energy, local_view_id){
    ...
    document.getElementById(`expl_fuel_co2${local_view_id}`).innerHTML = `${(default_background_values['fuel_details'][energy]['co2_g_liter'].toString()).replaceAll('.', ',')} CO²g/litra`;
    document.getElementById(`expl_water_temp_cold${local_view_id}`).innerHTML = `Temperatura vode (hladna): ${default_temperature_values['temperature_cold']} °C`;
    document.getElementById(`expl_water_temp_warm${local_view_id}`).innerHTML = `Temperatura vode (topla): ${default_temperature_values['temperature_warm']} °C`;
    document.getElementById(`expl_water_cost_cold${local_view_id}`).innerHTML = `Cijena hladne vode: ${(default_background_values['cold_water_price'].toString()).replaceAll('.', ',')} €/m³`;
    document.getElementById(`expl_water_cost_waste${local_view_id}`).innerHTML = `Cijena otpadnih voda: ${(default_background_values['waste_water_price'].toString()).replaceAll('.', ',')} €/m³`;
}
...
function writeCalculations(local_view_id){
    ...
    document.getElementById(`amount_saving${local_view_id}`).innerHTML = Intl.NumberFormat('de-DE').format(calcSavingMoneyDiff(local_view_id));
    document.getElementById(`water_saving${local_view_id}`).innerHTML = Intl.NumberFormat('de-DE').format(calcSavingWater());
    document.getElementById(`co2_saving${local_view_id}`).innerHTML = Intl.NumberFormat('de-DE').format(calcSavingCo2Diff(local_view_id));
    document.getElementById(`price_total${local_view_id}`).innerHTML = Intl.NumberFormat('de-DE').format(calcTotalPrice(local_view_id));
    ...
    document.getElementById(`amotization_amount${local_view_id}`).innerHTML = Intl.NumberFormat('de-DE').format(calcAmortizationAmount(local_view_id));
...
function calcAmortizationTime(local_view_id){
    ...
    if (amo_time_value < 1){
        amo_time_string = "< 1 Mjesec";
        amo_two_time_string = "prvog mjeseca";
        return [amo_time_string, amo_two_time_string]

    } else if(Math.round(amo_time_value) === 1){
        amo_time_string = `${Math.round(amo_time_value)} Mjesec`;
        amo_two_time_string = `${Math.round(amo_time_value)} Mjesec`
        return [amo_time_string, amo_two_time_string]

    } else {
        amo_time_string = `${Math.round(amo_time_value)} Mjeseci`;
        amo_two_time_string = `${Math.round(amo_time_value)} mjeseca`;
        return [amo_time_string, amo_two_time_string]
    }
}
