...
function checkShower(value, local_view_id){
    ...
    if(value <= 0 || Number.isInteger(value) === false){
        alert("Il numero deve essere almeno 1");
        value = 1;
    } 
...
function readCheckBox(local_view_id){
    ...
    if (user_input[`connection${local_view_id}`] > 0){
        ...
        alert("Selezionare almeno una connessione!");
    }
}
...
function setExplanation(energy, local_view_id){
    ...
    document.getElementById(`expl_fuel_co2${local_view_id}`).innerHTML = `${(default_background_values['fuel_details'][energy]['co2_g_liter'].toString()).replaceAll('.', ',')} CO²g/Litri`;
    document.getElementById(`expl_water_temp_cold${local_view_id}`).innerHTML = `Temperatura dell'acqua (fredda): ${default_temperature_values['temperature_cold']} °C`;
    document.getElementById(`expl_water_temp_warm${local_view_id}`).innerHTML = `Temperatura dell'acqua (calda): ${default_temperature_values['temperature_warm']} °C`;
    document.getElementById(`expl_water_cost_cold${local_view_id}`).innerHTML = `Prezzo dell'acqua fredda: ${(default_background_values['cold_water_price'].toString()).replaceAll('.', ',')} €/m³`;
    document.getElementById(`expl_water_cost_waste${local_view_id}`).innerHTML = `Prezzo delle acque reflue: ${(default_background_values['waste_water_price'].toString()).replaceAll('.', ',')} €/m³`;
}
...
function calcAmortizationTime(local_view_id){
    ...
    if (amo_time_value < 1){
        amo_time_string = "< 1 mese";
        amo_two_time_string = "del primo mese";
        return [amo_time_string, amo_two_time_string]

    } else if(Math.round(amo_time_value) === 1){
        amo_time_string = `${Math.round(amo_time_value)} Mese`;
        amo_two_time_string = `${Math.round(amo_time_value)} Mese`
        return [amo_time_string, amo_two_time_string]
