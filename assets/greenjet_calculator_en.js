...
function setExplanation(energy, local_view_id){
    ...
    document.getElementById(`expl_water_temp_cold${local_view_id}`).innerHTML = `Water temperature(cold): ${default_temperature_values['temperature_cold']} °C`;
    document.getElementById(`expl_water_temp_warm${local_view_id}`).innerHTML = `Water temperature(warm): ${default_temperature_values['temperature_warm']} °C`;
    document.getElementById(`expl_water_cost_cold${local_view_id}`).innerHTML = `Cold water price: ${(default_background_values['cold_water_price'].toString()).replaceAll('.', ',')} €/m³`;
    document.getElementById(`expl_water_cost_waste${local_view_id}`).innerHTML = `Waste water price: ${(default_background_values['waste_water_price'].toString()).replaceAll('.', ',')} €/m³`;
}
...
function calcAmortizationTime(local_view_id){
    ...
    if (amo_time_value < 1){
        amo_time_string = "< 1 month";
        amo_two_time_string = "of the first month";
        return [amo_time_string, amo_two_time_string]

    } else if(Math.round(amo_time_value) === 1){
        amo_time_string = `${Math.round(amo_time_value)} month`;
        amo_two_time_string = `${Math.round(amo_time_value)} month`
        return [amo_time_string, amo_two_time_string]

    } else {
        amo_time_string = `${Math.round(amo_time_value)} months`;
        amo_two_time_string = `${Math.round(amo_time_value)} months`;
        return [amo_time_string, amo_two_time_string]
    }
}
