...
function setExplanation(energy, local_view_id){
    ...
    document.getElementById(`expl_fuel_co2${local_view_id}`).innerHTML = `${(default_background_values['fuel_details'][energy]['co2_g_liter'].toString()).replaceAll('.', ',')} CO²g/Litros`;
    document.getElementById(`expl_water_temp_cold${local_view_id}`).innerHTML = `Temperatura del agua (fría): ${default_temperature_values['temperature_cold']} °C`;
    document.getElementById(`expl_water_temp_warm${local_view_id}`).innerHTML = `Temperatura del agua (templada): ${default_temperature_values['temperature_warm']} °C`;
    document.getElementById(`expl_water_cost_cold${local_view_id}`).innerHTML = `Precio del agua fría: ${(default_background_values['cold_water_price'].toString()).replaceAll('.', ',')} €/m³`;
    document.getElementById(`expl_water_cost_waste${local_view_id}`).innerHTML = `Precio de las aguas residuales: ${(default_background_values['waste_water_price'].toString()).replaceAll('.', ',')} €/m³`;
}
...
function calcAmortizationTime(local_view_id){
    ...
    if (amo_time_value < 1){
        amo_time_string = "< 1 mes";
        amo_two_time_string = "del primer mes";
        return [amo_time_string, amo_two_time_string]

    } else if(Math.round(amo_time_value) === 1){
        amo_time_string = `${Math.round(amo_time_value)} Mes`;
        amo_two_time_string = `${Math.round(amo_time_value)} Mes`
        return [amo_time_string, amo_two_time_string]

    } else {
        amo_time_string = `${Math.round(amo_time_value)} Meses`;
        amo_two_time_string = `${Math.round(amo_time_value)} Meses`;
        return [amo_time_string, amo_two_time_string]
    }
}
