...
function setupEventExplanation(local_view_id){
    ...
    let visibility = document.getElementById(`calculation_explanation${local_view_id}`).display = "none";
...
function setExplanation(energy, local_view_id){
    ...
    document.getElementById(`expl_water_temp_cold${local_view_id}`).innerHTML = `Wassertemperatur(kalt): ${default_temperature_values['temperature_cold']} °C`;
    document.getElementById(`expl_water_temp_warm${local_view_id}`).innerHTML = `Wassertemperatur(warm): ${default_temperature_values['temperature_warm']} °C`;
    document.getElementById(`expl_water_cost_cold${local_view_id}`).innerHTML = `Kaltwasserpreis: ${(default_background_values['cold_water_price'].toString()).replaceAll('.', ',')} €/m³`;
    document.getElementById(`expl_water_cost_waste${local_view_id}`).innerHTML = `Abwasserpreis: ${(default_background_values['waste_water_price'].toString()).replaceAll('.', ',')} €/m³`;
}
...
function calcAmortizationTime(local_view_id){
    ...
    if (amo_time_value < 1){
        ...
        amo_time_string = `${Math.round(amo_time_value)} Monate`;
        amo_two_time_string = `${Math.round(amo_time_value)} Monaten`;
        return [amo_time_string, amo_two_time_string]
    }
}
